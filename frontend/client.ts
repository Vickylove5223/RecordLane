 ...
 * Import the endpoint handlers to derive the types for the client.
 */
import { create as api_metadata_create_create } from "~backend/metadata/create";
import { deleteRecording as api_metadata_delete_deleteRecording } from "~backend/metadata/delete";
import { get as api_metadata_get_get } from "~backend/metadata/get";
import { list as api_metadata_list_list } from "~backend/metadata/list";
import { update as api_metadata_update_update } from "~backend/metadata/update";
...
export namespace metadata {
    ...
    export class ServiceClient {
         ...
         */
        public async create(params: RequestType<typeof api_metadata_create_create>): Promise<ResponseType<typeof api_metadata_create_create>> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/recordings`, {method: "POST", body: JSON.stringify(params)})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_metadata_create_create>
        }
        ...
        public async deleteRecording(params: { id: string }): Promise<void> {
            await this.baseClient.callTypedAPI(`/recordings/${encodeURIComponent(params.id)}`, {method: "DELETE", body: undefined})
        }

        /**
         * Retrieves a recording by its ID.
         */
        public async get(params: { id: string }): Promise<ResponseType<typeof api_metadata_get_get>> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/recordings/${encodeURIComponent(params.id)}`, {method: "GET", body: undefined})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_metadata_get_get>
        }

        /**
         * Retrieves all recordings, ordered by creation date (latest first).
         */
        public async list(params: RequestType<typeof api_metadata_list_list>): Promise<ResponseType<typeof api_metadata_list_list>> {
            // Convert our params into the objects we need for the request
            const query = makeRecord<string, string | string[]>({
            ...
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/recordings`, {query, method: "GET", body: undefined})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_metadata_list_list>
        }

        /**
         * Updates a recording's metadata.
         */
        public async update(params: RequestType<typeof api_metadata_update_update>): Promise<ResponseType<typeof api_metadata_update_update>> {
            ...
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_metadata_update_update>
        }
    }
}
