...
_ErrorHandler.errorLog = [];
_ErrorHandler.maxLogSize = 100;
_ErrorHandler.listeners = [];
let ErrorHandler = _ErrorHandler;
ErrorHandler.initialize();
const initialState = {
  isOnboarded: true,
  // Skip onboarding to avoid requiring users to save data repeatedly
  settingsOpen: false,
      ...
      case "UPDATE_RECORDING":
        const updatedRecordingState = {
          ...
          recordings: state.recordings.map(
            (recording) => recording.id === action.payload.id ? { ...recording, ...action.payload.updates } : recording
          )
        };
...
function AppProvider({ children }) {
  const [state, dispatch2] = reactExports.useReducer(appReducer, initialState);
  const refreshRecordings = React.useCallback(async () => {
    try {
      ...
      dispatch2({ type: "SET_ERROR", payload: "Failed to refresh recordings" });
    } finally {
  ...
  React.useEffect(() => {
    const loadState = async () => {
      try {
          ...
          if (savedState) {
            try {
              ...
              if (parsed.recordings) {
                const syncedRecordings = parsed.recordings.filter(
                  (rec) => rec.uploadStatus === "completed" && rec.youtubeLink
                );
                parsed.recordings = syncedRecordings.map((rec) => ({
                  ...rec,
                  createdAt: new Date(rec.createdAt)
                }));
              }
              parsed.isOnboarded = true;
              dispatch2({ type: "LOAD_STATE", payload: parsed });
            } catch (error) {
        ...
        dispatch2({ type: "SET_ERROR", payload: "Failed to load application state" });
      } finally {
        dispatch2({ type: "SET_LOADING", payload: false });
      }
    };
  ...
  React.useEffect(() => {
    if (state.error) {
      ...
      const timeoutId = setTimeout(() => {
        dispatch2({ type: "SET_ERROR", payload: null });
      }, 1e4);
      return () => clearTimeout(timeoutId);
    }
  }, [state.error]);
  return /* @__PURE__ */ jsxRuntimeExports.jsx(AppContext.Provider, { value: { state, dispatch: dispatch2, refreshRecordings }, children });
}
function useApp() {
...
class RetryService {
  ...
  async executeWithBackoff(operation, maxRetries = 3, baseDelay = 1e3) {
  ...
  async executeWithLinearBackoff(operation, maxRetries = 3, delayIncrement = 1e3) {
    return this.execute(operation, {
      maxRetries,
  ...
  async executeImmediate(operation, maxRetries = 2) {
    return this.execute(operation, {
      maxRetries,
      retryDelay: 0,
      backoffMultiplier: 1
  ...
  async executeWithCondition(operation, shouldRetry, maxRetries = 3, delay = 1e3) {
    return this.execute(operation, {
      maxRetries,
      retryDelay: delay,
      shouldRetry
...
class PerformanceMonitor {
  constructor() {
    ...
    this.suggestionCooldown = 3e5;
    this.initializeMonitoring();
  }
  // 5 minutes cooldown for suggestions
  static getInstance() {
    if (!this.instance) {
  ...
  getOptimizationSuggestions() {
    ...
    if (metrics.networkLatency && metrics.networkLatency > 5e3) {
  ...
  cleanup() {
    ...
    if (this.metricsCollectionInterval) {
      ...
      this.metricsCollectionInterval = void 0;
    }
    if (typeof window !== "undefined" && window.performance) {
      try {
...
class CacheService {
  constructor(namespace, maxSize = CACHE_CONFIG.maxAge) {
    this.hitCount = 0;
    this.missCount = 0;
  ...
  async evictIfNecessary(cacheData) {
    const stats = this.calculateCacheSize(cacheData);
    if (stats.totalSize > this.maxSize) {
      const entries = Object.entries(cacheData).map(([key, entry]) => ({ key, entry })).sort((a, b) => {
...
const _GlobalCacheManager = class _GlobalCacheManager {
  static getInstance(namespace, maxSize) {
...
const _RealYouTubeService = class _RealYouTubeService {
  ...
  static async checkConnection() {
    ...
    try {
      ...
      if (!tokenData || !tokenData.accessToken) {
        const result2 = { isConnected: false, userEmail: null };
        await this.cache.set("connection-status", result2, 30 * 1e3);
      ...
      await this.cache.set("connection-status", result, 5 * 60 * 1e3);
      ...
      await this.cache.set("connection-status", result, 30 * 1e3);
      this.notifyConnectionChange(false);
      return result;
    }
  }
  ...
  static async uploadVideo(blob, title, privacyStatus, onProgress) {
    await this.initialize();
    try {
      const tokenData = this.getStoredTokenData();
  ...
  static async openOAuthPopup(authUrl, expectedState) {
...
_RealYouTubeService.cache = new CacheService("youtube-service");
_RealYouTubeService.retryService = new RetryService();
_RealYouTubeService.connectionListeners = [];
_RealYouTubeService.oauthConfig = null;
_RealYouTubeService.initializationPromise = null;
let RealYouTubeService = _RealYouTubeService;
if (typeof window !== "undefined") {
  RealYouTubeService.handleOAuthCallback();
}
const realYouTubeService = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  RealYouTubeService
}, Symbol.toStringTag, { value: "Module" }));
const _FrontendYouTubeService = class _FrontendYouTubeService {
  static addConnectionListener(listener) {
    this.connectionListeners.push(listener);
    return () => {
      const index = this.connectionListeners.indexOf(listener);
  ...
  static async uploadVideo(file, title, privacy, onProgress) {
    const tokenData = this.getStoredTokenData();
    if (!tokenData) {
    ...
    try {
      const initResponse = await fetch("https://www.googleapis.com/upload/youtube/v3/videos?uploadType=resumable&part=snippet,status", {
        ...
        body: JSON.stringify({
          snippet: {
            title,
            description: `Uploaded from RecordLane - ${(/* @__PURE__ */ new Date()).toISOString()}`,
            tags: ["RecordLane", "Screen Recording"],
            categoryId: "22"
            // People & Blogs
          },
  ...
  static storeTokenData(tokenData) {
    try {
...
_FrontendYouTubeService.cache = /* @__PURE__ */ new Map();
_FrontendYouTubeService.retryService = new RetryService();
_FrontendYouTubeService.connectionListeners = [];
let FrontendYouTubeService = _FrontendYouTubeService;
const YouTubeContext = reactExports.createContext(void 0);
function YouTubeProvider({ children }) {
  ...
  const disconnectYouTube = reactExports.useCallback(async () => {
    try {
      if (isYouTubeConfigured()) ;
      else {
  ...
  const uploadVideo = reactExports.useCallback(async (file, title, privacy, onProgress) => {
    if (!isConnected) {
  ...
  const refreshToken = reactExports.useCallback(async () => {
    try {
      let refreshed;
      if (isYouTubeConfigured()) ;
      else {
  ...
  const retryConnection = reactExports.useCallback(async () => {
    if (connectionError) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(YouTubeContext.Provider, { value: {
    isConnected,
    userEmail,
    isConnecting,
    connectionError,
...
function useYouTube() {
  const context = reactExports.useContext(YouTubeContext);
  if (context === void 0) {
...
class RecordingService {
  ...
  async startRecording(options) {
    try {
      ...
      if (this.dataAvailableTimeout) {
        ...
        this.dataAvailableTimeout = void 0;
      }
  ...
  async checkAndRequestPermissions(options) {
    try {
  ...
  checkBrowserSupport() {
    return !!(navigator.mediaDevices && navigator.mediaDevices.getUserMedia && window.MediaRecorder && window.MediaStream);
  }
  async setupStreams(options) {
  ...
  async setupMediaRecorder(options) {
  ...
  static async checkPermissions(mode, microphone = false) {
    const permissions = {
      camera: false,
      microphone: false,
      screenCapture: false
...
const RecordingContext = reactExports.createContext(void 0);
function RecordingProvider({ children }) {
  ...
  const [permissionStatus, setPermissionStatus] = reactExports.useState({
    camera: "prompt",
    microphone: "prompt",
    screen: "prompt"
  });
  ...
  const checkPermissions = reactExports.useCallback(async () => {
    if (!navigator.mediaDevices) {
    ...
    try {
      ...
      try {
        ...
        setPermissionStatus((prev) => ({
          ...prev,
          camera: cameraPermission.state === "granted" ? "granted" : cameraPermission.state === "denied" ? "denied" : "prompt"
        }));
      } catch (error) {
        try {
      ...
      try {
        ...
        setPermissionStatus((prev) => ({
          ...prev,
          microphone: microphonePermission.state === "granted" ? "granted" : microphonePermission.state === "denied" ? "denied" : "prompt"
        }));
      } catch (error) {
        try {
      ...
      const hasScreenCapture = navigator.mediaDevices.getDisplayMedia !== void 0;
      setPermissionStatus((prev) => ({
        ...prev,
  ...
  const requestPermissions = reactExports.useCallback(async (type) => {
    try {
  ...
  }, [startTimer, toast2, permissionStatus, requestPermissions]);
  ...
  const updateOptions = reactExports.useCallback((newOptions) => {
    setOptions((prev) => ({ ...prev, ...newOptions }));
  }, []);
  const getPreviewUrl = reactExports.useCallback(() => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(RecordingContext.Provider, { value: {
    state,
    duration,
    recordedBlob,
    options,
...
function useRecording() {
  const context = reactExports.useContext(RecordingContext);
  if (context === void 0) {
...
class ErrorBoundary extends reactExports.Component {
  constructor(props) {
    ...
    this.maxRetries = 3;
    this.checkConnectionStatus = async () => {
  ...
  render() {
    if (this.state.hasError) {
      ...
      return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "min-h-screen bg-background flex items-center justify-center p-4", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(Card, { className: "max-w-2xl w-full max-h-[90vh] overflow-hidden", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs(CardHeader, { className: "text-center pb-4", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-16 h-16 bg-destructive/10 rounded-full flex items-center justify-center mx-auto mb-4", children: /* @__PURE__ */ jsxRuntimeExports.jsx(TriangleAlert, { className: "h-8 w-8 text-destructive" }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs(CardTitle, { className: "text-xl flex items-center justify-center gap-2", children: [
            "Something went wrong",
            /* @__PURE__ */ jsxRuntimeExports.jsx(Badge, { variant: severity === "critical" ? "destructive" : severity === "high" ? "destructive" : "secondary", children: severity })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-center gap-4 text-sm text-muted-foreground", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-1", children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: new Date(this.state.lastErrorTime).toLocaleTimeString() })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-1", children: [
              this.state.connectionStatus === "online" ? /* @__PURE__ */ jsxRuntimeExports.jsx(Wifi, { className: "h-4 w-4 text-green-500" }) : this.state.connectionStatus === "offline" ? /* @__PURE__ */ jsxRuntimeExports.jsx(WifiOff, { className: "h-4 w-4 text-red-500" }) : /* @__PURE__ */ jsxRuntimeExports.jsx(Monitor, { className: "h-4 w-4 text-yellow-500" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "capitalize", children: this.state.connectionStatus })
            ] }),
            this.state.retryCount > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-1", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs(CardContent, { className: "space-y-4", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-center text-muted-foreground", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "mb-2", children: "RecordLane encountered an unexpected error and needs to restart." }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm", children: this.state.connectionStatus === "offline" ? "Please check your internet connection and try again." : "If this keeps happening, please try the suggestions below." })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "p-3 bg-muted rounded-lg", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-sm", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: "Error ID:" }),
            " ",
            /* @__PURE__ */ jsxRuntimeExports.jsx("code", { className: "bg-background px-1 rounded", children: this.state.errorId })
          ] }) }),
          suggestions.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("h4", { className: "font-medium text-sm", children: "Suggestions:" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("ul", { className: "text-sm text-muted-foreground space-y-1", children: suggestions.map((suggestion, index) => /* @__PURE__ */ jsxRuntimeExports.jsxs("li", { className: "flex items-start gap-2", children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col sm:flex-row gap-2 pt-4", children: [
            canRetry && /* @__PURE__ */ jsxRuntimeExports.jsx(
              Button,
              {
                ...
                variant: "outline",
                className: "flex-1",
                children: this.state.isRetrying ? /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
...
function withErrorBoundary(Component2, errorBoundaryProps) {
  const WrappedComponent = (props) => /* @__PURE__ */ jsxRuntimeExports.jsx(ErrorBoundary, { ...errorBoundaryProps, isolateError: true, children: /* @__PURE__ */ jsxRuntimeExports.jsx(Component2, { ...props }) });
...
function LoadingSpinner({ text, size = "md", className }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: cn("flex items-center space-x-2", className), children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(Spinner, { size }),
...
function AppLoadingFallback() {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "min-h-screen bg-background flex items-center justify-center", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-center", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { className: "text-2xl font-bold mb-2", children: "RecordLane" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(LoadingSpinner, { text: "Loading application..." })
  ] }) });
}
function AppInner() {
  ...
  const [lastSuggestionLog, setLastSuggestionLog] = reactExports.useState(0);
  reactExports.useEffect(() => {
    markStart("app-init");
    ...
    const unsubscribe = performanceMonitor.addObserver((newMetrics) => {
      ...
      const now = Date.now();
      if (suggestions.length > 0 && now - lastSuggestionLog > 6e4) {
  ...
  if (!isInitialized) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx(AppLoadingFallback, {});
  }
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "min-h-screen bg-background text-foreground", children: /* @__PURE__ */ jsxRuntimeExports.jsx(ErrorBoundary, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(AppProvider, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(ErrorBoundary, { isolateError: true, children: /* @__PURE__ */ jsxRuntimeExports.jsx(YouTubeProvider, { children: /* @__PURE__ */ jsxRuntimeExports.jsx(ErrorBoundary, { isolateError: true, children: /* @__PURE__ */ jsxRuntimeExports.jsx(RecordingProvider, { children: /* @__PURE__ */ jsxRuntimeExports.jsxs(BrowserRouter, { children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(reactExports.Suspense, { fallback: /* @__PURE__ */ jsxRuntimeExports.jsx(AppLoadingFallback, {}), children: /* @__PURE__ */ jsxRuntimeExports.jsxs(Routes, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(Route, { path: "/youtube-setup", element: /* @__PURE__ */ jsxRuntimeExports.jsx(YouTubeSetupPage, {}) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(Route, { path: "/recording/:recordingId", element: /* @__PURE__ */ jsxRuntimeExports.jsx(VideoSharePage, {}) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(Route, { path: "*", element: /* @__PURE__ */ jsxRuntimeExports.jsx(AppShell, {}) })
    ] }) }),
...
function checkBrowserCompatibility() {
  ...
  const missingAPIs = Object.entries(requiredAPIs).filter(([, supported]) => !supported).map(([api]) => api);
...
function App() {
  return /* @__PURE__ */ jsxRuntimeExports.jsx(AppInner, {});
}
