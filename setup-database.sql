...
CREATE POLICY IF NOT EXISTS "Public read access" ON recordings
  FOR SELECT USING (true);

CREATE POLICY IF NOT EXISTS "Authenticated users can insert" ON recordings
  FOR INSERT WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY IF NOT EXISTS "Users can update their own recordings" ON recordings
  FOR UPDATE USING (auth.role() = 'authenticated');

CREATE POLICY IF NOT EXISTS "Users can delete their own recordings" ON recordings
  FOR DELETE USING (auth.role() = 'authenticated');

-- Create policies for events
CREATE POLICY IF NOT EXISTS "Public insert access for analytics" ON events
  FOR INSERT WITH CHECK (true);

CREATE POLICY IF NOT EXISTS "Authenticated users can read events" ON events
